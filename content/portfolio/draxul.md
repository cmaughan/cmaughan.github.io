...
At its centre is a per-user server that owns shells, spaces, panes, and agents independently of the UI. A client can disconnect, close, and later reattach without losing the work behind it. That makes Draxul useful as a harness for long-running agentic sessions: the processes keep going, while one or more lightweight clients provide the view onto them.
